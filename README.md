...
Built to be a model for small agencies serving immigrant and underserved communities: **the server can never read an application**, the form works on a phone in English or Spanish for people who read little, and every piece is small enough for one volunteer to maintain.

## How it fits together

```
site/        static site, English and Spanish, built by Python + Jinja2 → GitHub Pages
  apply      the form: encrypts in the browser (age) to the season's public key
...
An application travels: phone → encrypted → Worker (ciphertext) → review tool (decrypts with the key that only lives on the board's Mac) → decisions → status back to the Worker (a word, never a reason) → purge after distribution.
